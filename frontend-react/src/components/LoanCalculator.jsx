...
const LoanCalculator = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const { principal, downPayment, tradeIn } = formData;

    const principalValue = Number(principal);
    const downPaymentValue = Number(downPayment);
    const tradeInValue = Number(tradeIn);

    if (downPaymentValue < 0 || tradeInValue < 0 || principalValue < 0) {
      alert("Down Payment or Trade-In or principal value cannot be negative");
      return;
    }

    if (downPaymentValue > principalValue || tradeInValue > principalValue) {
      alert(
        "Down Payment or Trade-In value cannot be greater than the vehicle price"
      );
      return;
    }
  ...
  const handleGetRequest = async () => {
    try {
      ...
      const response = await fetch(
        `https://axlr8-backend-kyxs.onrender.com/api/v1/loan/calculate-loan?${queryParams}`,
        // `http://localhost:8081/api/v1/loan/calculate-loan?${queryParams}`,
        {
          method: "GET",
          headers: {},
        }
  ...
  return (
    <>
      <div className="flex flex-col items-center lg:flex-row">
        <form
          className="max-w-md mx-auto mb-8 lg:mb-10 mt-20"
